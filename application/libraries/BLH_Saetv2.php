...
class SaeTOAuthV2 {
    ...
    /**
     * 授权查询|查询用户access_token的授权相关信息，包括授权时间，过期时间和scope权限。
     * @ignore
     */
    function getTokenInfoURL()    { return 'https://api.weibo.com/oauth2/get_token_info'; }
    /**
     * 替换授权|OAuth1.0的Access Token更换至OAuth2.0的Access Token，帮助开发者使用新版接口和OAuth2.0时平滑迁移用户。
     * @ignore
     */
    function getOauth2TokenURL()    { return 'https://api.weibo.com/oauth2/get_oauth2_token'; }
    /**
     * 授权回收|授权回收接口，帮助开发者主动取消用户的授权
     * @ignore
     */
    function revokeoauth2URL()    { return 'https://api.weibo.com/oauth2/revokeoauth2'; }
    /**
     * 获取微博开放平台内部Wiki的Host
     * @ignore
     */
    function getInternalHost($path = '2')    { return 'https://m.api.weibo.com/'.$path.'/'; }
...
class SaeTClientV2
{
     ...
     * @return array
     */
    function getUserInfo( $uid, $access_token = '', $app_key = WEIBO_APP_KEY)
    {
        $params=array();
        if ($access_token) {
        ...
        $ret = $this->oauth->get('users/show', $params );
        if (!empty($ret) && !isset($ret['error_code']))
        {
            unset($ret['class'],$ret['description'],$ret['url'],$ret['weihao'],$ret['status'],$ret['ptype'],$ret['lang'],$ret['star'],$ret['mbtype'],$ret['mbrank'],$ret['block_word']);
        }
        return $ret;
    }
     ...
     * @return array
     */
    function account_profile_basic( $uid = NULL, $access_token = '', $app_key = WEIBO_APP_KEY)
    {
        $params = array();
        if ($access_token) {
        	$params['access_token'] = $access_token;
        ...
        $ret = $this->oauth->get( 'account/profile/basic', $params );
        return $ret;
    }
    ...
    function get_token_info($token)
    {
        ...
        return $this->oauth->post($this->oauth->getTokenInfoURL(), $params);
    }
    /**
     * 设置微博weibo的内部访问接口
     * 
     * @access public
     * @param string $host 请求微博weibo的接口host
     * @param string $path 接口路径
     * @see http://wiki.intra.weibo.com/2/messages/invite
     * @return void
     */
    function set_internal_host($path = '2')
    {
        $this->oauth->host = $this->oauth->getInternalHost($path);
    }
    /**
     * 
     * @param $type
    ...
    function getAccessToken( $type = 'code', $keys )
    {
        return $this->oauth->getAccessToken($type, $keys);
    }
    /**
     * 批量获取用户的基本信息
     ...
     */
    function getBatchUserInfo($uids, $page=1, $pagesize=20)
    {
        ...
        $user_all_info = array();
        $totalPage = ceil($uid_count / $pagesize);
        do
        {
            $uids_loop = ($uid_count <= $pagesize) ? $uids : array_slice($uids, ($page-1)*$pagesize, $pagesize);
            $user_all_list = $this->users_show_batch_by_id($uids_loop);
            if (isset($user_all_list['users']) && is_array($user_all_list['users']) && !empty($user_all_list['users']))
            {
                foreach($user_all_list['users'] as $user_item)
                {
                    unset($user_item['class'],$user_item['description'],$user_item['url'],$user_item['weihao'],$user_item['status'],$user_item['ptype'],$user_item['lang'],$user_item['star'],$user_item['mbtype'],$user_item['mbrank'],$user_item['block_word']);
                    $user_all_info[$user_item['idstr']] = $user_item;
                }
            }
            ++ $page;
        }
     ...
     * @param int $uid 需要获取双向关注列表的用户UID。
     * @param int $page 当前页数,默认1
     * @param int $pagesize 单页返回的记录条数，默认为50，最大不超过2000。
     * @see http://open.weibo.com/wiki/2/friendships/friends/bilateral/ids
     * @return [ids:{互粉的UID列表}, total_number:互粉的UID总数]
     */
    function getAllFriendIds($uid, $page=1, $pagesize=2000)
    {
        $user_all_friends_info = $this->bilateral_ids($uid, $page, $pagesize);
        //接口返回的好友ID数
        $count = (int)$user_all_friends_info['total_number'];
        //实际的ids中的好友ID数小于返回的好友ID数
        while (count($user_all_friends_info['ids']) < $count)
        {
            ++ $page;
            $user_all_friends_info_new = $this->bilateral_ids($uid, $page, $pagesize);
            if (count($user_all_friends_info_new['ids']) == 0)
            {
                break;
            }
            $user_all_friends_info['ids'] = array_unique(array_merge($user_all_friends_info_new['ids'], $user_all_friends_info['ids']));
        }
        return $user_all_friends_info;
    }
    /**
     * 获取用户关注的用户UID列表
     *
     ...
     * @access public
     * @param int $uid 要获取的用户 UID，默认为当前用户
     * @param int $page 返回结果的游标，下一页用返回值里的next_cursor，上一页用previous_cursor，默认为0。
     * @param int $pagesize 每次返回的最大记录数（即页面大小），不大于5000, 默认返回500。
     * @return array
     */
    function get_friends_ids_by_id($uid, $page=1, $pagesize=5000)
    {
        $user_all_friends_ids_info = $this->friends_ids_by_id($uid, $page - 1, $pagesize);
        //接口返回的好友ID数
        $count = (int)$user_all_friends_ids_info['total_number'];
        //返回结果的游标，下一页
        $next_cursor = isset($user_all_friends_ids_info_new['next_cursor']) ? (int)$user_all_friends_ids_info['next_cursor'] : $page;
        //实际的ids中的好友ID数小于返回的好友ID数
        while (count($user_all_friends_ids_info['ids']) < $count)
        {
            ++ $page;
            $user_all_friends_ids_info_new = $this->friends_ids_by_id($uid, $next_cursor, $pagesize);
            if (count($user_all_friends_ids_info_new['ids']) == 0)
            {
                break;
            }
            $user_all_friends_ids_info['ids'] = array_unique(array_merge($user_all_friends_ids_info_new['ids'], $user_all_friends_ids_info['ids']));
            //返回结果的游标，下一页
            $next_cursor = isset($user_all_friends_ids_info_new['next_cursor']) ? (int)$user_all_friends_ids_info_new['next_cursor'] : $page;
        }
        return $user_all_friends_ids_info;
    }
    /**
     * 好友邀请接口-批量[微博开放平台内部WIKI]
     *
     * 对应API：{@link http://wiki.intra.weibo.com/2/messages/invite}
     *
     * @param string $uids 被邀请人，需为当前用户互粉好友。。
     * @param string $data 邀请数据。必须为json字串的形式，必须做URLEncode，采用UTF-8编码。
     * @example data里面包含的参数格式
     *  text string true    邀请私信内容，文本大小+参数url大小必须小于300个汉字。
     *  url string  false   邀请点击后跳转链接。默认为当前应用地址。
     *  invite_logo string  false   邀请Card展示时的图标地址，大小必须为80px X 80px，仅支持PNG、JPG格式。默认为当前应用logo地址。
     * @return array
     */
    function send_invite_batch( $uids, $params)
    {
        $uid_list = array_unique(Utilities::filter($uids));
        $result = array();
        if (is_array($uid_list) && !empty($uid_list))
        {
            $this->set_internal_host();
            foreach ($uid_list as $uid)
            {
                if (strlen($uid) <= 0)
                {
                    $result['failures'][] = array('uid'=>$uid);
                    continue;
                }
                $ret = $this->send_invite($uid, $params);
                if (is_array($ret) && !empty($ret) && !isset($ret['error_code']) && !isset($ret['error']))
                {
                    if (!isset($result['data']))
                    {
                        $result['data'] = isset($ret['data']) ? $ret['data'] : array();
                        $result['data']['text'] = isset($ret['text']) ? $ret['text'] : '';
                    }
                    unset($ret['text'], $ret['data']);
                    $result['invitation'][] = $ret;
                }
                else
                {
                    $result['failures'][] = array('uid'=>$uid) + $ret;
                }
            }
        }
        return $result;
     ...
     * 对应API：{@link http://wiki.intra.weibo.com/2/messages/invite}
     *
     * @param string $uid 被邀请人，需为当前用户互粉好友。。
     * @param string $data 邀请数据。必须为json字串的形式，必须做URLEncode，采用UTF-8编码。
     * @return array
     */
    function send_invite( $uid , $data)
    {
        $params = array();
        $this->id_format($uid);
        $params['uid'] = $uid;
     ...
     * 喜欢某个对象/赞接口
     *
     * 对应API：{@link http://wiki.intra.weibo.com/2/likes/update}
     * 
     * @param string $uid 被邀请人，需为当前用户互粉好友。
     * @return array
     */
    function likesUpdate($appkey, $access_token = '', $object_id, $object_type = 'game', $with_activitie = 0)
    {
        $params = array();
        //$this->id_format($uid);
        $params['access_token'] = $access_token;
        ...
        $params['with_activitie'] = $with_activitie;
        return $this->oauth->post( 'likes/update', $params );
    }
