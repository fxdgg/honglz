...
<body class="mainPage jdDetails">
    ...
    <div class="main " id="jd_content"> 
        <div class="maintip">
            <h2><?php echo isset($data['baseInfo']['company'])?$data['baseInfo']['company']:'';?> 招聘 <?php echo $displayPosLevel;?></h2>
        </div>
        <div class="details">
            <h3>岗位职责：</h3>
            <?php
            ...
            <?php
                $tr_content = '';
                if (isset($data['demand']) && !empty($data['demand']))
                {
                    foreach ($data['demand'] as $key => $value)
                    {
                        $p = $key+1;
                        $value = str_replace(array('"'),'',$value);
                        /*$tr_content .= '<p id="p_' . $p . '">
    ...
    <div class="footer">
        <div class="con">
            ...
            <div class="bns">
                <a href="#" class="white_bn copy">一键复制</a><a href="<?php echo APP_SITE_URL;?>/j/m/<?php echo $rstep;?>/<?php echo $staticPageLevel;?>.html" class="blue_bn">生成静态页</a>
            </div>
        </div>
    </div>
    </form>
