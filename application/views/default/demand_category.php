...
<div class="info">
    ...
    <div class="infoTop">
        <h1>
            <?php if($companyTypeList){foreach ($companyTypeList as $ctItem){?>
                <a href="/demand/category?ctid=<?php echo $ctItem['companyTypeId']; ?>"><?php echo $ctItem['companyTypeName']; ?></a>&nbsp;&nbsp;&nbsp;&nbsp;
            <?php }}?>
        </h1>
    </div>
</div>
<table width='100%' cellspacing='1' cellpadding='2' class="table table-striped table-hover table-condensed  table-bordered" style='table-layout: fixed;overflow: hidden;' bgcolor="#BBDDE5">
    ...
    <?php if($list){foreach ($list as $data){?>
        <tr id="tr_<?php echo $data['id'];?>" height="22" bgcolor="#FFFFFF" onMouseOver="this.className='over'" onMouseOut="this.className='out'">
            <td align="center" title="<?php echo $data['title'];?>"><div style="overflow:hidden;"><a href="/demand/info?id=<?php echo $data['id'];?>" targe="_blank"><?php echo $data['title'];?></a></div></td>
            <td align="center" ><div style="overflow:hidden;"><?php echo $data['updateDate'];?></div></td>
        </tr>
    <?php }}?>
    </tbody>
    <tfoot>
