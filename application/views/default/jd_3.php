        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                var randomHtml = '<div id="div_'+data.kid+'" class="msg">'+
                                    ...
                                    '<div class="fns">'+
                                        '<span class="more" onclick="randomJdContent(this,'+id+','+level+','+kid+','+data.id+');"></span>'+
                                        '<span class="del" onclick="removeTrOpra(this,'+data.id+','+data.kid+');"></span>'+
                                    '</div>'+
                                '</div>';
                //jQuery("#show_table_id").append(randomHtml);
                // jQuery("#div_"+data.kid).html(randomHtml);
...
function posMsg( $msg ){
    var offset = $msg.offset(),  pos = $(window).height()- 100 - offset.top-$msg.height();
...
<body class="mainPage">
    <div class="header">
        <div class="con">
            ...
            <div class="steps step2">
                ...
                <span class="step s3 on">任职资格<span>3</span></span>
            </div>
        </div>
    </div>
    ...
    <div class="main">
        ...
        <div class="maincon">
            <h3 class="title"><?php echo $displayPosLevel;?> <span>任职资格：</span></h3>
            <div class="msgs">
            <?php
                $tr_content = '';
                if (!empty($describeList))
                {
                    ...
                    foreach ($describeList as $key_kid => $value)
                    {
                        ...
                        $tr_content .= '<div id="div_' . $kid . '" class="msg">
                            ...
                            <textarea style="height:75px;" name="content['.$aid.']" id="content_' . $aid . '" readonly="true" onclick="editTrCommand(this,'.$aid.');" onblur="onBlurCommand(this,'.$aid.')">' . '（'.$value['keyword'].'）' . $value['content'] . '</textarea>
                            <div class="fns">
                                <span class="more" onclick="randomJdContent(this,'.$id.','.$level.','.$kid.','.$aid.');"></span>
                                <span class="del" onclick="removeTrOpra(this,'.$aid.','.$kid.');"></span>
                            </div>
                        </div>';
                    }
    ...
    <div class="footer">
        <div class="con">
            ...
            <div class="bns">
                <a href="<?php echo APP_SITE_URL;?>/j/m/<?php echo $lstep;?>?i=<?php echo $id;?>&l=<?php echo $level;?>" class="white_bn">上一步</a><a href="#" class="blue_bn" onclick="nextBtnSubmit();">生成职位介绍</a>
            </div>
        </div>
    </div>
    </form>
