...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function get($table = '', $limit = null, $offset = null)
	{
		...
		$this->_reset_select();
		//Add by gf 2013-11-11 02：00
		log_message('debug', 'METHOD: '.__METHOD__.'|sql: ' . $sql);
		//log_message('debug', 'METHOD: '.__METHOD__.'|sql: ' . $sql . ' result: ' . var_export($result->result_array(), true));
		return $result;
	}
	...
	public function count_all_results($table = '')
	{
		...
		$row = $query->row();
		$row_num =  (int) $row->numrows;
		//Add by gf 2013-11-11 02：00
		log_message('debug', 'METHOD: '.__METHOD__.'|sql: ' . $sql . ' row_num: ' . $row_num . ' result: ' . var_export($row, true));
		return $row_num;
	}
	 ...
	 * @return	object
	 */
	function insert($table = '', $set = NULL, $onDuplicate = NULL, $ignore = FALSE)
	{
		if ( ! is_null($set))
		{
			$this->set($set);
		...
		$sql = $this->_insert($this->_protect_identifiers($table, TRUE, NULL, FALSE), array_keys($this->ar_set), array_values($this->ar_set), $onDuplicate, $ignore);

		$this->_reset_write();
		$ret = $this->query($sql);
		//Add by gf 2013-11-11 02：00
		log_message('debug', 'METHOD: '.__METHOD__.'|sql: ' . $sql . ' result: ' . var_export($ret, true));
		return $ret;
	}
